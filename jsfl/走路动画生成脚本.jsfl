...
if(importPath != null) {
	...
	//包含各个方向子文件夹的父文件夹
	//譬如待机、跑步
	var fatherFolders = new Array("待机", "跑步", "坐下", "俯卧撑");
	//frame per png，隔5帧 0 5 10
	var fppngs = new Array(5, 3, 3, 3);
	//最后一帧是否stop
	var lastFrameScripts = new Array("", "", "stop();", "gotoAndPlay(12);");
		...
		for (var i=0; i<directions.length; i++) {
			...
			if (fileList) {
				...
				for(var j=0; j<fileList.length; j++) {
					...
					lib.addItemToDocument({x:0, y:-46}, pngimg);
					//将图片移动到方向目录
