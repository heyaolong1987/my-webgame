...
package flashx.textLayout.elements 
{
	...
	public final class ParagraphElement extends ParagraphFormattedElement
	{
		...
		* @playerversion AIR 1.5
		...
		{
			var pos:int = relativePosition-1;
			//可能是自动换行插入了一张图片，删除该图片导致该行没了
			try {
				var textLine:TextLine = (getTextBlock().getTextLineAtCharIndex(relativePosition));
				if(textLine != null) {
					pos = getTextBlock().findPreviousAtomBoundary(relativePosition);
				}
			}
			catch(e:Error) {
				trace(e, relativePosition);
			}
			return pos;
		}
